...
const PAGE_SIZE = 8;
const SESSION_KEY = 'bookIndex_v3';
...
export default function BookIndex() {
      ...
      try {
        ...
        const results = await Promise.allSettled(
          files.map(async (file) => {
            const r = await fetch(`/books/${file}`);
            if (!r.ok) return null;
            const text = await r.text();
            if (!text.trim()) return null;
            const book = safeJsonParse(text) ?? {
              title: 'Unknown Title',
            ...
            return [file.replace('.json', ''), book] as const;
          })
        );
  ...
  // SAVE STATE ON EVERY CHANGE + BEFORE LEAVING
  useEffect(() => {
    const state = { currentPage, searchQuery, sortBy, selectedContributors, scrollY: window.scrollY };
  ...
  useEffect(() => {
    const saveOnLeave = () => {
      sessionStorage.setItem(SESSION_KEY, JSON.stringify({
        currentPage, searchQuery, sortBy, selectedContributors, scrollY: window.scrollY
      }));
    };
    window.addEventListener('beforeunload', saveOnLeave);
    const handleVisibilityChange = () => { if (document.visibilityState === 'hidden') saveOnLeave(); };
  ...
  // RESTORE STATE — WORKS ON ANDROID + VERCEL
  useEffect(() => {
    if (loading || restoreAttempted.current) return;
    ...
    try {
      ...
      if (Array.isArray(saved.selectedContributors)) setSelectedContributors(saved.selectedContributors);
      if (saved.currentPage) setCurrentPage(saved.currentPage);

      // Critical: wait for layout, then restore scroll
      setTimeout(() => {
        if (saved.scrollY > 100) {
  ...
  const filteredContributors = useMemo(() => {
    ...
    return allContributors.filter(c => c.toLowerCase().includes(contributorSearch.toLowerCase()));
  ...
  const filteredBooks = useMemo(() => {
    ...
    if (selectedContributors.length && sortBy !== 'sharedBy') {
      entries = entries.filter(([, b]) => b.sharedBy?.trim() && selectedContributors.includes(b.sharedBy.trim()));
    }
  ...
  const sortedBookList = useMemo(() => {
    ...
    if (sortBy === 'author') list.sort((a, b) => (a[1].author || '').localeCompare(b[1].author || ''));
    if (sortBy === 'sharedBy') list.sort((a, b) => (a[1].sharedBy || '').localeCompare(b[1].sharedBy || ''));
    return list;
  }, [filteredBooks, sortBy]);

  const { displayed, currentContributor, totalPages, contributorList } = useMemo(() => {
    if (sortBy !== 'sharedBy') {
    ...
    sortedBookList.forEach(item => {
      const key = item[1].sharedBy?.trim() || 'Unknown';
      (groups[key] ||= []).push(item);
    });
    const contribs = Object.keys(groups).filter(k => groups[k].length).sort((a, b) => a.localeCompare(b));
    const idx = Math.max(0, Math.min(currentPage - 1, contribs.length - 1));
    const name = contribs[idx] || 'Unknown';

    return {
      displayed: groups[name] || [],
      currentContributor: name,
      totalPages: contribs.length || 1,
      contributorList: contribs,
    };
    ...
    <>
      ...
      <section className="py-20 px-6 bg-black">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="max-w-2xl mx-auto flex gap-4 mb-12">
            ...
            {sortBy !== 'sharedBy' && allContributors.length > 0 && (
              <button onClick={() => setShowContributorModal(true)}
                ...
                {selectedContributors.length > 0 && <span className="ml-2 px-3 py-1 bg-amber-600/30 rounded-full text-xs">{selectedContributors.length}</span>}
              </button>
            )}
          </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && setShowContributorModal(false)}>
              <div className="w-full max-w-lg mx-4 bg-gradient-to-b from-amber-950/90 to-black border border-amber-800/60 rounded-2xl shadow-2xl overflow-hidden">
                <div className="p-8">
                  ...
                  <input type="text" value={contributorSearch} onChange={e => setContributorSearch(e.target.value)}
                    placeholder="Search contributors..." className="w-full mb-6 px-5 py-4 bg-amber-950/50 border border-amber-800/60 rounded-xl text-amber-100 placeholder-amber-500/60 focus:outline-none focus:border-amber-500" />
                  <div className="max-h-96 overflow-y-auto space-y-3">
                    {filteredContributors.map(name => (
                      <label key={name} className="flex items-center gap-4 p-4 rounded-lg hover:bg-amber-900/30 transition cursor-pointer">
                        <input type="checkbox" checked={selectedContributors.includes(name)}
                          onChange={e => setSelectedContributors(p => e.target.checked ? [...p, name] : p.filter(n => n !== name))}
                  ...
                  <div className="flex gap-4 mt-8">
                    <button onClick={() => { setSelectedContributors([]); setShowContributorModal(false); }}
                      className="flex-1 py-4 border border-amber-700/70 text-amber-300 hover:bg-amber-900/30 rounded-xl transition">Clear All</button>
                    <button onClick={() => setShowContributorModal(false)}
          ...
          {sortBy === 'sharedBy' && currentContributor && (
            <div className="text-center mb-12">
              <h3 className="text-4xl font-serif text-amber-200">Books shared by: <span className="text-amber-400 font-bold">{currentContributor}</span></h3>
              <p className="text-amber-400 text-lg mt-4">Contributor {currentPage} of {totalPages}</p>
            </div>
          )}

          {sortedBookList.length === 0 ? (
            <div className="text-center py-20"><p className="text-amber-400 text-2xl">No books found.</p></div>
          ) : (
            <>
                ...
                <div className="flex justify-center items-center gap-6 mt-16">
                  ...
                  <span className="text-amber-300 text-lg min-w-[200px] text-center">
                    ...
                    {sortBy === 'sharedBy' && contributorList[currentPage - 1] && (
                      <span className="block text-sm text-amber-400 mt-1">({contributorList[currentPage - 1]})</span>
                    )}
                  </span>
      ...
      {showScrollTop && (
        <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="fixed bottom-8 right-8 z-50 p-4 bg-amber-500/90 hover:bg-amber-400 rounded-full shadow-2xl transition animate-pulse">
